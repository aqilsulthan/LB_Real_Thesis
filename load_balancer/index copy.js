...
const validAlgorithms = ['sa', 'hs', 'sahsh'];
...
app.get('/api/:endpoint', async (req, res) => {
    ...
    if (!req.query.algo || !validAlgorithms.includes(algo.toLowerCase())) {
        return res.status(400).json({ error: 'Invalid or missing algorithm', details: 'Algorithm must be one of: sa, hs, sahsh' });
    }
    ...
    try {
        ...
        // Ambil load nyata dari respons aplikasi
        const task = response.data.load || executionTime;
